...
public:
    ...
    void set_point_size(int size) { point_size_ = size; }
    void set_intensity_color(bool intensity_show) {
        intensity_color_ = intensity_show;
    }
    void set_overlap_filter(bool filter_mode) { overlap_filter_ = filter_mode; }
    ...
    cv::Mat ProjectToRawMat(cv::Mat img, cv::Mat K, cv::Mat D, cv::Mat R,
        ...
        cv::Mat projCloud2d = K * (R_ * cloud_ + repeat(T_, 1, cloud_.cols));
        float maxDist = 0;
    ...
    cv::Mat ProjectToRawImage(cv::Mat img, Eigen::Matrix3d K,
        ...
        // cv::Mat img_ = cv::imread(imgName);
        return ProjectToRawMat(img, K1, D1, R1, T1);
    }

    cv::Mat project_to_fisheye_mat(cv::Mat img, cv::Mat K, cv::Mat D, cv::Mat R,
                                   cv::Mat T) {
        cv::Mat I = cv::Mat::eye(3, 3, CV_32FC1);
        cv::Mat mapX, mapY;
        cv::Mat outImg = cv::Mat(img.size(), CV_32FC3);
    ...
    cv::Mat project_fisheye(std::string imgName, cv::Mat K, cv::Mat D,
                            cv::Mat R, cv::Mat T) {
        cv::Mat img = cv::imread(imgName);
        return project_to_fisheye_mat(img, K, D, R, T);
    }
};